...
class ImageLogger(Callback):
    ...
    def _wandb(self, pl_module, images, batch_idx, split):
        for k in images:
            ...
            grid_image = Image.fromarray(((grid_np)*255).astype('uint8'), mode='RGB')

            tag = f"{split}-vis/{k}"

            # pl_module.logger.experiment.log({tag: wandb.Image(grid_image), "global_step": pl_module.global_step})
            pl_module.logger.experiment.log({tag: [wandb.Image(grid_image)]})
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if ((split == "val") and (self.log_all_val)):
            ...
            inputs = images['inputs'].clone().detach().cpu()
            lpips_inputs = torch.clamp(inputs, -1., 1.)
            inputs = torch.clamp(inputs, -1., 1.).permute(0, 2, 3, 1)
            inputs = (inputs + 1.0) / 2.0
            inputs = inputs.numpy()
            inputs = (inputs * 255).astype(np.uint8)
            samples = images['samples'].clone().detach().cpu()
            lpips_samples = torch.clamp(samples, -1., 1.)
            samples = torch.clamp(samples, -1., 1.).permute(0, 2, 3, 1)
            samples = (samples + 1.0) / 2.0
            samples = samples.numpy()
            samples = (samples * 255).astype(np.uint8)
            cfg = images['samples_cfg_scale_3.00'].clone().detach().cpu()
            lpips_cfg = torch.clamp(cfg, -1., 1.)
            cfg = torch.clamp(cfg, -1., 1.).permute(0, 2, 3, 1)
            cfg = (cfg + 1.0) / 2.0
            cfg = cfg.numpy()
            cfg = (cfg * 255).astype(np.uint8)
            ...
            for i in range(inputs.shape[0]):
                img1 = Image.fromarray(inputs[i]).convert('L')
                img2 = Image.fromarray(samples[i]).convert('L')
                img3 = Image.fromarray(cfg[i]).convert('L')
                tot_ssim += ssim(np.array(img1), np.array(img2), data_range=255)
                tot_psnr += psnr(inputs[i], samples[i], data_range=255)
                tot_ssim1 += ssim(np.array(img1), np.array(img3), data_range=255)
                tot_psnr1 += psnr(inputs[i], cfg[i], data_range=255)
                tot_lpips += loss_fn_alex(lpips_inputs[i], lpips_samples[i])
                tot_lpips1 += loss_fn_alex(lpips_inputs[i], lpips_cfg[i])

            tot_lpips = tot_lpips * 1.0 / inputs.shape[0]
            tot_lpips1 = tot_lpips1 * 1.0 / inputs.shape[0]
            tot_ssim = tot_ssim * 1.0 / inputs.shape[0]
            tot_ssim1 = tot_ssim1 * 1.0 / inputs.shape[0]
            tot_psnr = tot_psnr * 1.0 / inputs.shape[0]
            tot_psnr1 = tot_psnr1 * 1.0 / inputs.shape[0]

            tmp_dict = {}
            tmp_dict['lpips_samples'] = tot_lpips
            tmp_dict['lpips_cfg'] = tot_lpips1
            tmp_dict['ssim_samples'] = tot_ssim
            tmp_dict['ssim_cfg'] = tot_ssim1
            tmp_dict['psnr_samples'] = tot_psnr
            tmp_dict['psnr_cfg'] = tot_psnr1
            pl_module.logger.experiment.log(tmp_dict, step=pl_module.global_step)
            ...
            if is_train:
                pl_module.train()
        # breakpoint()
        if ((split=="train") and should_log and  (check_idx % self.batch_freq == 0) and
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        # breakpoint()
        if not self.disabled and pl_module.global_step > 0 and batch_idx == 0 and (pl_module.current_epoch % 5 == 0):
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            ...
            in_filters_load = old_state["model.diffusion_model.input_blocks.0.0.weight"]#(320,4,3,3)
            new_state = model.state_dict()
            in_filters_current = new_state["model.diffusion_model.input_blocks.0.0.weight"]#(320,8,3,3)
            rank_zero_print(in_filters_current.shape)
            ...
            if in_shape != in_filters_load.shape:
                ...
                for input_key in input_keys:
                    if input_key not in old_state or input_key not in new_state:
                        continue
                    input_weight = new_state[input_key]#(320,8,3,3)
                    if input_weight.size() != old_state[input_key].size():
                        print(f"Manual init: {input_key}")
                        input_weight.zero_()
                        input_weight[:, :4, :, :].copy_(old_state[input_key])
                        # if input_weight.shape[1] == 16:
                        #     input_weight[:, 8:12, :, :].copy_(old_state[input_key])
                        old_state[input_key] = torch.nn.parameter.Parameter(input_weight)

            if config.model.params.mask_super or config.model.params.mask_super1:
                out_filters_load = old_state["model.diffusion_model.out.2.weight"]#(320,4,3,3)
                out_filters_current = new_state["model.diffusion_model.out.2.weight"]#(320,8,3,3)
                rank_zero_print(out_filters_current.shape)
                ...
                if out_shape != out_filters_load.shape:
                    ...
                    output_weight = new_state["model.diffusion_model.out.2.weight"]#(320,8,3,3)
                    output_bias = new_state["model.diffusion_model.out.2.bias"]

                    if output_weight.size() != old_state["model.diffusion_model.out.2.weight"].size():
                        print(f"Manual init: model.diffusion_model.out.2.weight")
                        output_weight.zero_()
                        output_weight[:4, :, :, :].copy_(old_state["model.diffusion_model.out.2.weight"])
                        old_state["model.diffusion_model.out.2.weight"] = torch.nn.parameter.Parameter(output_weight)
                    
                    if output_bias.size() != old_state["model.diffusion_model.out.2.bias"].size():
                        print(f"Manual init: model.diffusion_model.out.2.bias")
                        output_bias.zero_()
                        output_bias[:4].copy_(old_state["model.diffusion_model.out.2.bias"])
                        old_state["model.diffusion_model.out.2.bias"] = torch.nn.parameter.Parameter(output_bias)
        ...
        print(logdir)
        default_logger_cfg = default_logger_cfgs[config.logger.name]
