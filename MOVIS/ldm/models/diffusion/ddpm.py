...
def cape_embed(rel, feat):
    m = cape(feat, rel)
    q = (feat * m.cos()) + (rotate_every_two(feat) * m.sin())
    return q

def cape_encoder(feat_map, rel_pose):
    rel_pose = einops.repeat(rel_pose, 'b t_out d -> b (t_out l) d', l=feat_map.shape[1])
    encode_feat = cape_embed(rel_pose, feat_map)
    return encode_feat
...
class DDPM(pl.LightningModule):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        tmp = torch.ones(x.shape[0], device=self.device)
        avg = self.num_timesteps
        var = 200
        t = torch.nn.init.trunc_normal_(tmp, avg, var, 0, self.num_timesteps).long()
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if not self.cape:
            ...
            self.cc_projection = nn.Linear(1040, 768)
            nn.init.kaiming_normal_(self.cc_projection.weight, mode='fan_in')
            nn.init.zeros_(self.cc_projection.bias)
            self.cc_projection.requires_grad_(True)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        with torch.enable_grad():
            ...
            if not self.cape:
                cond["c_crossattn"] = [self.cc_projection(torch.cat([torch.where(prompt_mask, null_prompt, clip_emb), T2[:, None, :]], dim=-1))]
            else:
                feat_map = torch.where(prompt_mask, null_prompt, clip_emb)
                rel = T2[:, None, :]
                rel_pose = einops.repeat(rel, 'b l n -> b (l k) n', k=feat_map.shape[1])
                # m = einops.repeat(p, 'b l n -> b l (n k)', k=d // n)
    ...
    def forward(self, x, c, xx, *args, **kwargs):
        # breakpoint()
        tmp_epoch = 0
        mm = self.num_timesteps
        if self.schedule:
            for callback in self.trainer.callbacks:
                if isinstance(callback, ImageLogger):
                    # breakpoint()
                    tmp_epoch = callback.epoch
            if (tmp_epoch < 55):
                mm = self.num_timesteps
            elif (tmp_epoch <= 80):
                mm = self.num_timesteps // 2 + (80 - tmp_epoch) / 25 * (self.num_timesteps // 2)
            elif (tmp_epoch > 80):
                mm = self.num_timesteps // 2
            # if (tmp_epoch < 56):
            #     print(mm)
        t = self.timestep_sampler(x, mm)
        ...
        return self.p_losses(x, c, t, xx, *args, **kwargs)
    ...
    def apply_model(self, x_noisy, t, cond, xx_noisy=None, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            # breakpoint()
            if (xx_noisy is not None):
                x_recon = self.model(x_noisy, t, xx_noisy, **cond)
            else:
    ...
    def p_losses(self, x_start, cond, t, xx_start, noise=None):
        # breakpoint()
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            if (len(tmp_out) == 2):
                samples, intermediates = tmp_out
            else:
                samples, intermediates, mm = tmp_out
    ...
    def configure_optimizers(self):
        ...
        if not self.cape:
            ...
            if self.cc_projection is not None:
                ...
                opt = torch.optim.AdamW([{"params": self.model.parameters(), "lr": lr}], lr=lr)
        if self.use_scheduler:
    ...
    def timestep_sampler(self, x, mm):
        if self.training and self.timestep_sampling_cfg.target == 'Gaussian':
            tmp = torch.ones(x.shape[0], device=self.device)
            # avg = self.num_timesteps
            avg = mm
            var = self.timestep_sampling_cfg.params.var
            return torch.nn.init.trunc_normal_(tmp, avg, var, 0, self.num_timesteps - 1e-3).long()
            # t = torch.normal(avg, var, size=(x.shape[0],), device=self.device).long()#revise to gaussian sampling
        else:
            return torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, xx = None, c_concat: list = None, c_crossattn: list = None, depth1: list = None, mask_cond: list = None, c_adm=None):
        ...
        if self.conditioning_key is None:
            ...
            # breakpoint()
            if (depth1 is not None and mask_cond is not None):
                xc = torch.cat([x] + c_concat + mask_cond + depth1, dim=1)
            elif (xx is not None and mask_cond is not None):
